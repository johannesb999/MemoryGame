...
let highScores = [];  // This should be loaded from persistent storage

// Updates the dropdown to reflect the current high scores
function updateHighScoresDropdown() {
  const highScoresDropdown = document.getElementById('highScoresDropdown');

  // Clear existing high score entries
  while(highScoresDropdown.firstChild) {
    highScoresDropdown.removeChild(highScoresDropdown.firstChild);
  }

  // Add each high score to the dropdown
  highScores.forEach((highScore, index) => {
    const option = document.createElement('option');
    option.value = index;
    option.textContent = `Player: ${highScore.player}, Score: ${highScore.score}`;
    highScoresDropdown.appendChild(option);
  });
}

// Call this function when a game ends
function onGameEnd() {
  // Insert logic here to add the new high score, if it is one
  const newHighScore = { player: "Player 1", score: secondsElapsed };

  // Add the new score to the high scores array
  highScores.push(newHighScore);

  // Sort the array from lowest to highest score
  highScores.sort((a, b) => a.score - b.score);

  // Only keep the top 10 scores
  highScores = highScores.slice(0, 10);

  // Update the high scores dropdown to reflect the new scores
